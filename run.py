...
from signals import step_function
y_step = step_function(t, 0.5)

shifted_t = t + shift
y_step_shifted = step_function(shifted_t, 0.5)

plt.figure()
plt.plot(t, y_step, label="step function")
plt.plot(shifted_t, y_step_shifted, label="shifted step function")
plt.xlabel("time")
plt.ylabel("amplitude")
plt.legend()
plt.grid(True)
